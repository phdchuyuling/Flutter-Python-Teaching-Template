...
class DatabaseService {
  ...
  // In-memory fallback for Web (sqflite not supported on web).
  final List<Record> _inMemoryRecords = [];

  // If not web, use sqflite database lazily.
  Database? _database;
  ...
  Future<Database> _initDatabase() async {
    String dbPath = path.join(await getDatabasesPath(), 'bookkeep.db');
  ...
  Future<int> insertRecord(Record record) async {
    if (kIsWeb) {
      final id = (_inMemoryRecords.isEmpty ? 1 : (_inMemoryRecords.last.id ?? 0) + 1);
      final rec = Record(
        id: id,
        type: record.type,
  ...
  Future<List<Record>> getRecords() async {
    if (kIsWeb) {
      return List<Record>.from(_inMemoryRecords);
    }
    final db = await _db;
  ...
  Future<int> deleteRecord(int id) async {
    if (kIsWeb) {
      _inMemoryRecords.removeWhere((r) => r.id == id);
      return 1;
    }
    final db = await _db;
