...
class _CurrencyConverterPageState extends State<CurrencyConverterPage> {
  ...
  void initState() {
    ...
    _fetchRatesFromBOT().catchError((e) {
      // 若抓取台灣銀行失敗，在 Web 環境下（或其它情況）啟用 fallback
      // 並提供一組合理的預設幣別，避免 UI 一直顯示載入中
      setState(() {
        _useFallback = true;
        _currencies = ['TWD', 'USD', 'EUR', 'JPY', 'HKD', 'CNY'];
        _from = 'TWD';
        _to = 'USD';
        _lastUpdated = null;
      });
  ...
  Future<void> _convert() async {
    ...
    try {
      if (_from == _to) {
        setState(() {
          _resultText =
              '${NumberFormat('#,##0.00').format(amount)} $_from = ${NumberFormat('#,##0.00').format(amount)} $_to';
        });
        return;
      }
      ...
      setState(() {
        _resultText =
            '${f.format(amount)} $_from = ${f.format(computeResult)} $_to';
      });
    } catch (e) {
      setState(() {
        _resultText = '轉換失敗：$e';
  ...
  Future<double> _convertViaExchangeHost(
    double amount,
    String from,
    String to,
  ) async {
  ...
  Future<void> _fetchRatesFromBOT() async {
    ...
    if (kIsWeb) {
      final proxy =
          'https://api.allorigins.win/raw?url=${Uri.encodeComponent(original)}';
      uri = Uri.parse(proxy);
    }
    try {
      ...
      setState(() {
        ...
        if (!_currencies.contains(_to)) {
          _to = _currencies.length > 1 ? _currencies[1] : 'TWD';
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('匯率兌換器')),
      body: Padding(
          ...
          children: [
            ...
            if (_currencies.length <= 1)
              ...
              Row(
                children: [
                  Expanded(
                    child: _buildCurrencyDropdownWithLabel(
                      '來源幣別',
                      _from,
                      (v) => setState(() => _from = v!),
                    ),
              ...
              children: [
                Expanded(
                  child: ElevatedButton(
                    onPressed: (_loading || _currencies.length <= 1)
                        ? null
                        : _convert,
                    child: _loading
                        ? const SizedBox(
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('已更新台灣銀行匯率')),
                            );
                          } catch (e) {
            ...
            if (_lastUpdated != null)
              Text(
                '最後更新：${DateFormat('yyyy-MM-dd HH:mm').format(_lastUpdated!)}',
  ...
  Widget _buildCurrencyDropdownWithLabel(
    String label,
    String value,
